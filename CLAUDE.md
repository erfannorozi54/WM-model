...
- A thesis chapter (attention-gate suppression + proxy-pretraining as a "neural efficiency" finding, distinct from the accuracy/performance result already shown in `slidev-presentation/` — the proxy-pretraining accuracy gain is *not* a capacity claim, since it was never tested at higher N-back or larger simultaneous item counts) is written up in `docs/FUTURE_WORK_NEURAL_EFFICIENCY.md`, with real GPU results (not just a plan) in `docs/EXECUTION_PLAN_NEURAL_EFFICIENCY.md` §6–7. The two human-neuroscience references this chapter leans on (Poppenk, Moscovitch & McIntosh 2016; Constantinidis & Klingberg 2016) are already summarized in full, from the source PDFs, in `docs/PAPER_EXPLAINED_POPPENK_2016.md` and `docs/PAPER_EXPLAINED_CONSTANTINIDIS_KLINGBERG_2016.md` — read those before re-deriving a paper summary from scratch. `slidev-presentation/speaker_notes_neural_efficiency_onward.md` has the matching per-slide talking points for that same chapter. Check these before re-deriving similar analysis/metric designs from scratch.
