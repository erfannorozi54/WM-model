...
2. **Effective dimensionality / participation ratio.** Run PCA on the (trials × H) hidden-state matrix per cell; compute the participation ratio `PR = (Σλᵢ)² / Σλᵢ²`. Lower PR under familiarity would mirror the "sharpened/less-broadly-distributed" tuning Constantinidis & Klingberg describe after training.
3. **Population sparsity.** Fraction of hidden units with near-zero activation (below a small threshold relative to that unit's own max) or a Gini coefficient over unit activations. A more "efficient" code should be sparser, not merely smaller in magnitude.
4. **Trial-to-trial variability (Fano-factor analogue).** For repeated presentations of matched conditions (same `task_index`, `n`, `location`/`identity`/`category` combination across different trials), compute `Var(activation)/Mean(activation)` per unit, averaged over units — this is the direct RNN analogue of the Fano-factor reduction reported after WM training.
...
Everything above operates on RNN hidden-state activity (`hidden`), which is the only quantity the baseline model exposes. The attention-enhanced architecture (`AttentionWorkingMemoryModel` / `FeatureChannelAttention`, `src/models/attention.py`) exposes something strictly stronger: an explicit, literal per-channel gate in `[0, 1]` computed *before* the RNN even sees the CNN features. A gate near 0 on a task-irrelevant channel is not a proxy for suppression inferred from a norm — it **is** suppression, by construction. This maps onto feature-based-attention/distractor-suppression literature (Desimone & Duncan's biased competition; Treue & Martínez-Trujillo's feature-similarity gain model) more directly than repetition suppression does, and it is a signature the plain GRU/RNN baseline cannot produce at all, which makes it a stronger candidate for "a new finding only our model can show" than the magnitude/PR/sparsity metrics alone.
...
**Named for §4.8 (attention-gate suppression), NOT independently full-text verified — cite only after reading in full, the same standard applied to references 1–2 above:**
5. Desimone, R., & Duncan, J. (1995). Neural mechanisms of selective visual attention. *Annual Review of Neuroscience*, 18, 193–222. (Biased competition model — suppression of task-irrelevant stimulus representations under attentional control.)
6. Treue, S., & Martínez-Trujillo, J. C. (1999). Feature-based attention influences motion processing gain in macaque visual cortex. *Nature*, 399, 575–579. (Feature-similarity gain model — attention to one feature suppresses neural responses to other, task-irrelevant features across the visual field, not just at attended locations.)
