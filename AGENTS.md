...
0c. **Population metrics in `neural_efficiency.py` are only comparable at matched trial counts.** `participation_ratio` and threshold-based `population_sparsity` both grow with N on identical data (~+44% and ~+42% from N=50 to N=1600). `compare_cell()` emits `trial_count_warning` when `n_trials_a != n_trials_b` — do not quote those two rows when it fires. `fano_factor_analogue` is `Var/Mean` on a continuous signal and so scales **linearly with activation magnitude**, which is exactly what the magnitude metric reports differing between conditions; read `cv_squared` (scale-invariant) alongside it. `activation_magnitude` and `cv_squared` are unaffected by both issues.
